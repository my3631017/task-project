...
//                返回方法在下面定义
    ...
    public String addUser(User user, Model model) {
        if (user.getName().trim().equals("") || user.getNumber().trim().equals("")) {
...
//            userDaoService.updateUser(user)的返回值是true或者false
    ...
    public String findUsersByPage(String page, Model model) {
        try {
            if (userDaoService.findUsers().isEmpty()) {
                model.addAttribute("message", "数据库里没有任何东西！");
                return "message";
            }
            p.setTotalUsers(userDaoService.findUsers().size());
            if (Integer.parseInt(page) > 0 && Integer.parseInt(page) <= p.getTotalPages()) {
                p.setCurrentPage(Integer.parseInt(page));
                List<User> list = userDaoService.findUsersByPage((Integer.parseInt(page) - 1) * p.getPageSize(), p.getPageSize());
//            查询结果是list集合
                model.addAttribute("list", list);
                model.addAttribute("page", p);
                return "home";
            } else {
                model.addAttribute("message", "超出页面范围");
                return "message";
            }
        } catch (NumberFormatException e) {
            model.addAttribute("message", "输入错误或输入为空");
            return "message";
        } catch (Exception e) {
